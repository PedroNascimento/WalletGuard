...
export const DespesaFiltersComponent: React.FC<DespesaFiltersProps> = ({ filters, onFiltersChange }) => {
    const handleClearFilters = () => {
        onFiltersChange({});
    };

    const hasActiveFilters = Object.keys(filters).some(key => filters[key as keyof DespesaFilters]);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <select
                        value={filters.tipo || ''}
                        onChange={(e) => onFiltersChange({ ...filters, tipo: e.target.value as 'fixa' | 'variavel' | undefined })}
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-white"
                    >
