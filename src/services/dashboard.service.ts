...
export const dashboardService = {
  ...
  async getCurrentMonthSummary() {
    ...
    const totalDespesas =
      despesas?.reduce((sum, d) => sum + Number(d.value), 0) || 0;
  ...
  async getCardsTotalBill() {
    ...
    const total =
      data?.reduce((sum, expense) => sum + Number(expense.value), 0) || 0;
  ...
  async getCategorySummary() {
    ...
    // Agrupar receitas
    const receitasPorCategoria = (receitasData || []).reduce(
    ...
    // Agrupar despesas
    const despesasPorCategoria = (despesasData || []).reduce(
