...
export const cardsService = {
  ...
  async delete(id: string) {
    ...
    if (count === 0)
      throw new Error(
        "Erro ao excluir: Cartão não encontrado ou sem permissão.",
      );
    return true;
  },
  ...
  async createExpense(cardId: string, expenseData: CardExpenseFormData) {
    ...
    const installmentValue = Number(
      (expenseData.value / expenseData.installments).toFixed(2),
    );
    ...
    for (let i = 0; i < expenseData.installments; i++) {
      ...
      expenses.push({
        ...
        card_id: cardId,
        description:
          expenseData.installments > 1
            ? `${expenseData.description} (${i + 1}/${expenseData.installments})`
  ...
  async getInvoiceHistory(cardId: string) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const { summary } = await this.getExpenses(
        cardId,
        d.getMonth() + 1,
        d.getFullYear(),
      );
