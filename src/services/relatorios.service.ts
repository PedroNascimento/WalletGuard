...
export const relatoriosService = {
    async getDados(filters: RelatorioFilters) {
        ...
        if (filters.category) receitasQuery = receitasQuery.eq('categoria', filters.category);
        ...
        if (filters.category) despesasQuery = despesasQuery.eq('category', filters.category);
        ...
        if (filters.category) cardExpensesQuery = cardExpensesQuery.eq('category', filters.category);
        if (filters.cardId) cardExpensesQuery = cardExpensesQuery.eq('card_id', filters.cardId);

        const { data: cardExpensesData, error: cardError } = await cardExpensesQuery;
        ...
        const totalDespesasGerais = despesas.reduce((acc, curr) => acc + Number(curr.value), 0);
        const totalCardExpenses = cardExpenses.reduce((acc, curr) => acc + Number(curr.value), 0);
        const totalDespesas = totalDespesasGerais + totalCardExpenses;

        const maiorReceita = receitas.sort((a, b) => Number(b.valor) - Number(a.valor))[0];
        const todasDespesas = [
            ...despesas.map(d => ({ description: d.description, value: Number(d.value), date: d.date })),
            ...cardExpenses.map(d => ({ description: d.description, value: Number(d.value), date: d.date }))
        ];
        ...
        const resumo: ResumoFinanceiro = {
            ...
            maiorReceita: maiorReceita ? { description: maiorReceita.descricao, value: Number(maiorReceita.valor), date: maiorReceita.data } : { description: '-', value: 0, date: '' },
            maiorDespesa: maiorDespesa ? { description: maiorDespesa.description, value: maiorDespesa.value, date: maiorDespesa.date } : { description: '-', value: 0, date: '' }
        };
        ...
        receitas.forEach(r => {
            const date = r.data;
            const current = chartDataMap.get(date) || { name: format(parseISO(date), 'dd/MM'), receitas: 0, despesas: 0, saldo: 0 };
