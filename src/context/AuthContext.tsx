...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      console.log(
        "AuthContext: Sessão inicial obtida:",
  ...
  const syncUser = async (authUser: User) => {
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error("Timeout ao sincronizar usuário")),
          3000,
        );
      });
      ...
      await Promise.race([
        (async () => {
          console.log(
            "AuthContext: Verificando se usuário existe em app_users...",
          );
      ...
      console.warn(
        "AuthContext: Erro ou timeout ao sincronizar usuário (continuando login):",
        err,
      );
  ...
  const signOut = async () => {
    ...
    try {
      ...
      console.warn(
        "AuthContext: Erro ou timeout no signOut do Supabase (prosseguindo com limpeza local):",
        error,
      );
    } finally {
